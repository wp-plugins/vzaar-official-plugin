...
class Vzaar
{
     ...
     * @param int $page
     * @param string $sort
     * @param string $status - added to allow 'processing' videos to be displayed.
     * @return array <VideoList>
     */
	public static function searchVideoList($username, $auth = false, $title = '', $labels = '', $count = 20, $page = 1, $sort = 'desc', $status = 'active')
	{
		$_url = Vzaar::URL_LIVE . 'api/' . $username . '/videos.json?count=' . $count . '&page=' . $page . '&sort=' . $sort . '&status=' . $status;
	...
	public static function editVideo($id, $title, $description, $private = 'false', $seoUrl = '')
	{
		...
		$data = '<?xml version="1.0" encoding="UTF-8"?><vzaar-api><_method>put</_method><video><title><![CDATA[' . $title . ']]></title><description><![CDATA[' . $description . ']]></description>';
